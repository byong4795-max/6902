  ...
  if (urlParams.has('room')) {
    ...
    peer.on('open', id => {
      navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment' }, audio: false })
        .then(stream => {
          video = createCapture(VIDEO); // 手機端預覽
          video.elt.setAttribute('playsinline', '');
          video.hide();
          peer.call(roomId, stream);
        })
        .catch(err => console.error('手機端取得相機權限失敗', err));
    });
  } else {
    peer = new Peer(undefined, peerOptions);
    peer.on('open', id => {
...
function drawIntroScreen() {
  ...
  text(introText, width/2, height/2 - 50);
  
  fill(C_BLUE);
  rect(width/2, height - 100, 200, 50, 10);
  fill(255);
  textSize(22);
  text("準備好了！", width/2, height - 92);
}
...
function drawLevel1() {
  if (!video) { drawWaitingVideo(); return; }
...
function checkGestures() {
  ...
  if (predictions.length > 0 && millis() > cooldown) {
    ...
    // 手指指向判斷
    let dx = indexTip[0] - indexBase[0];
    let dy = indexTip[1] - indexBase[1];
    
    let mappedDir = -1;
    if (abs(dx) > abs(dy)) {
        // 鏡頭是鏡像的，所以 dx < 0 是右邊，dx > 0 是左邊
        if (dx < -40) mappedDir = 0; // 右
        if (dx > 40) mappedDir = 2;  // 左
    } else {
        if (dy < -40) mappedDir = 3; // 上
        if (dy > 40) mappedDir = 1;  // 下
    }

    lastDetectedDir = mappedDir;

    // 握拳判斷 (指尖靠近大拇指)
    let distFist = dist(indexTip[0], indexTip[1], thumbTip[0], thumbTip[1]);
    let isFist = distFist < 50;
...
function drawRestartBtn() {
  ...
  fill(255);
  textSize(22);
  text("重新挑戰", width/2, height - 92);
}

function drawWaitingVideo() {
  fill(C_BLUE);
  text("偵測不到鏡頭連線，請確保手機已掃碼...", width/2, height/2);
}
